...
public class LayoutConfig_PutUpdateTab {
	...
	public void Assert200(Hashtable<String,String> dataTable) {
		 ...
		 header("Authorization",auth.ValidAuth).body(file).when().put( dataTable.get("EndPoint")).
    ...
    public void Assert401(Hashtable<String,String> dataTable) {
		 ...
		 Response response = given().relaxedHTTPSValidation().
		 header("Authorization",auth.InvalidAuth).when().put( dataTable.get("EndPoint")).
		 then().assertThat().statusCode(401).and().extract().response();  
		 Assert.assertTrue(response.getTimeIn(TimeUnit.SECONDS)<=10,"Response Time is not within limit");
		 System.out.println(response.getTimeIn(TimeUnit.SECONDS));
