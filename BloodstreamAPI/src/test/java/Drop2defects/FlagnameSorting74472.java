...
public class FlagnameSorting74472 {
	...
	public void DonationInfoFlagnameSort(Hashtable<String,String> dataTable) {
		...
		public void TestInfoFlagnameSort(Hashtable<String,String> dataTable) {
			...
			String Authorization = config.property.getProperty("LoginToken");
			String endpoint = dataTable.get("EndPoint");
			Response response2 = given().relaxedHTTPSValidation().header("Authorization",Authorization).param("GroupStatus",dataTable.get("GroupStatus")).param("Sort",dataTable.get("Sort")).
			when().get(endpoint).then().assertThat().statusCode(200).extract().response();  
